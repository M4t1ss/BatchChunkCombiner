			...
			//Use the language model ONLY if the translations differ
				...
				switch($languageModel){
					...
					case 'RWTH':
						...
						$perplexities[] = trim(shell_exec('./getNN_PPL.sh '.$modelFile.' "'.$testPPLthree.'" '.$VOCAB.' '.$LM_TOOL_DIR));
						$perplexities[] = trim(shell_exec('./getNN_PPL.sh '.$modelFile.' "'.$testPPLfour.'" '.$VOCAB.' '.$LM_TOOL_DIR));
						break;
					case 'CharRNN':
